...
/* 3 - A professora da escola está com dificuldade de fechar a média dos alunos a tempo do fechamento do
semestre e precisa de ajuda. Para ajudá-la, você precisa fazer um programa que execute as seguintes
instruções:
a. Ler e mostrar nome;
b. Peça para digitar 2 notas;
c. Calcule a média do aluno;
d. Mostrar a média;
e. Com a média calculada, mostre a mensagem da situação do aluno em relação às seguintes
condições:
